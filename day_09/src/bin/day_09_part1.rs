...
fn find_next_value_in_sequence (sequence : &str) -> i64 {
    let seq: Vec<i64> =
    sequence.split(" ")
    ...
    loop {
        ...
        for i in  0..(history[depth].len() - 1) {
            if history[depth][i + 1].checked_sub(history[depth][i]) == None {
                println!("{:?}", sequence);
                println!("left ={}: right = {}", history[depth][i + 1], history[depth][i] );
                panic!("stop !!!!!!!!!!!!!!!!");
            }
    ...
    loop { // or while Ok(depth-=1)
        let next_pattern = history[depth + 1].last().unwrap() + history[depth].last().unwrap();
        history[depth].push(next_pattern);
    ...
    return history[0].last().unwrap().clone();
}
...
fn sum_of_extrapolated_values(sensor_reading: &str) -> i64 {
    ...
    sensor_reading.lines()
        ...
        .map(|line| find_next_value_in_sequence(line))
        .sum();
}
...
1 3 6 10 15 21
10 13 16 21 30 45";
        find_next_value_in_sequence("1 3 6 10 15 21");
        assert_eq!(114, sum_of_extrapolated_values(sensor_reading));
    } 
}